...
import { registerUser, updateUser } from '../services/api'; // ✅ updated to use updateUser
...
const AddEditTeacherModal: React.FC<AddEditTeacherModalProps> = ({ onClose, onSave, teacher }) => {
  const [formData, setFormData] = useState({
    ...
    password: teacher?.password || 'defaultPassword123',
    city: teacher?.city || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (teacher) {
        ...
        console.log('Updating teacher with ID:', teacher.id);
        ...
        const updateData: any = {
          ...
          is_affiliate: isAffiliate, // ✅ Key field
        };

        // Add affiliate fields only if isAffiliate is true
        if (isAffiliate) {
          updateData.affiliateId = formData.affiliateId;
          updateData.commissionPercentage = formData.commissionPercentage;
        }

        console.log('Sending update data:', updateData);

        const response = await updateUser(teacher.id, updateData);
        ...
        const apiData = {
          ...
          join_as: isAffiliate ? 'Both' : 'Teacher',
          affiliate_type: 'Individual',
          city: formData.city,
          address: formData.address || '',
          firm_name: '',
          specialization: formData.specialization,
          commissionPercentage: isAffiliate ? formData.commissionPercentage : 10.0,
        };
      ...
      setError(error.message || `Failed to ${teacher ? 'update' : 'register'} teacher. Please try again.`);
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="relative flex items-start pt-2">
            ...
            <div className="ml-3 text-sm">
              <label htmlFor="isAffiliate" className="font-medium text-gray-700">
          ...
          <div className="pt-4 flex justify-end gap-2">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? (teacher ? 'Updating...' : 'Registering...') : 'Save'}
            </button>
          </div>
        </form>
      </div>
